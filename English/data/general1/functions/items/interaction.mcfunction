...
execute at @a[scores={lp_drop=1..}] if entity @e[type=potion,nbt={Item:{id:"minecraft:lingering_potion",Count:1b,tag:{Tags:["smoke"]}}},distance=..2] run summon item ~ ~ ~ {Age:5800,PickupDelay:32767,Tags:["smoke"],Item:{id:"minecraft:stone_button",Count:1b}}
...
execute at @e[type=minecraft:armor_stand,tag=he] unless entity @e[distance=..2,type=potion,nbt={Item:{id:"minecraft:lingering_potion",Count:1b,tag:{Tags:["he"]}}}] run schedule function general1:items/reset_options 1t
...
#Only terrorists can pick up the bomb
data merge entity @e[type=item,nbt={Item:{id:"minecraft:tnt"}},limit=1] {Owner:[I;0,0,0,0]}
execute as @e[type=item,nbt={Item:{id:"minecraft:tnt"}}] at @s run data modify entity @s Owner set from entity @p[team=t] UUID

#Only counter-terrorists can pick up defuse kit
data merge entity @e[type=item,nbt={Item:{id:"minecraft:shears"}},limit=1] {Owner:[I;0,0,0,0]}
execute as @e[type=item,nbt={Item:{id:"minecraft:shears"}}] at @s run data modify entity @s Owner set from entity @p[team=ct] UUID

#Items owners
##################################################################################################################################
#Inventory is only hotbar
...
schedule function general1:items/interaction 1t
